...
sub doWith {
	...
	foreach $f (sort(glob("generated/*_1_*.xml"))) {
		...
		$f =~ s/_1_/_2_/;
		print FOUT "printf \"$f\\n\" >> time.txt\n";
		print FOUT "(time ../beast2 -overwrite -instances $threads -threads $threads ../$f >> screen2.dat) 2>>time.txt\n";
		print FOUT "printf \"\\n\" >> time.txt\n";
	}
}
...
sub doGPU {
	...
	foreach $f (sort(glob("generated/*_1_*.xml"))) {
		print FOUT "printf \"$f\\n\" >> time.txt\n";
		print FOUT "(time ../beast1 -overwrite -beagle_GPU -beagle_order $beagle_order ../$f  >> screen1.dat) 2>>time.txt\n";
		print FOUT "printf \"\\n\" >> time.txt\n";
		$f =~ s/_1_/_2_/;
		print FOUT "printf \"$f\\n\" >> time.txt\n";
		print FOUT "(time ../beast2 -overwrite -beagle_GPU ../$f  >> screen2.dat) 2>>time.txt\n";
		print FOUT "printf \"\\n\" >> time.txt\n";
	}
}
